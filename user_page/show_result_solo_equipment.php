...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $equipmentId = isset($_POST['equipmentId']) ? $_POST['equipmentId'] : '';
    $equipmentQty = isset($_POST['equipment_quantity']) ? $_POST['equipment_quantity'] : '';

    $equipment_qty = "$equipmentId:$equipmentQty,";
 

    // Display the retrieved form data
    $selectedItemsString = isset($_POST['selectedItems']) ? $_POST['selectedItems'] : '';

    // Define $itemFaciArray with appropriate value
    ...
    if ($result_file && $result_file->num_rows > 0) {
        ...
        $sql_request = "INSERT INTO request (equipment_qty, subject, date, purpose, user_id, file, status) VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql_request);

        // Use "i" for integer data types
        $stmt->bind_param("sssssis", $equipment_qty, $subject, $datepicker, $purpose, $user_id, $file_id, $value);
...
?>
<?php
    // Add this JavaScript code to show the modal after record deletion
    echo '<script>
