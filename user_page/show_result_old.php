...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($selectedItemsArray as $itemId) {
        ...
        $checkSql = "SELECT equipment_id, qty, facility_id FROM list_request WHERE id = $itemId";
        ...
        if ($checkResult->num_rows > 0) {
            ...
            if ($equipmentId != 0) {
                $itemQtyArray .= "$equipmentId:$qty,";
            }
    
            if ($facilityId != 0) {
                $itemFaciArray .= "$facilityId,";
            }

            $deleteSql = "DELETE FROM list_request WHERE id = $itemId";
...
<script>
    ...
    function redirectToPage() {
        window.location.href = 'frame_cart_list.php';
    }
