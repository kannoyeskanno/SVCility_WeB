...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get the request ID, status, and user ID from the POST data
    $requestId = $_POST["request_id"];
    $status = $_POST["status"];
    $userId = $_POST["user_id"]; // Assuming user_id is received through POST
    ...
    if ($stmtUpdate) {
        $stmtUpdate->bind_param("si", $status, $requestId); // Assuming status is a string and request_id is an integer

        if ($stmtUpdate->execute()) {
            // The status was successfully updated
            echo "Status updated to 'Submitted' successfully";
